...
public class TagServiceImpl implements TagService {
    ...
    public void createTag(List<TagCrawling> tags) {
        ...
        for (TagCrawling tagCrawl : tags) {
            ...
            List<Optional<Show>> shows = showRepository.findByTitleContaining(tagCrawl.getTitle());

            for (Optional<Show> show : shows) {
                if (show.isPresent()) {
                    // 공연 데이터를 받아오면 공연 태그 테이블에 값이 있는지 show 값과 tag 값으로 검색해본다.
                    Optional<ShowTag> showTag = showTagRepository.findByShowAndTag(show.get(), tag.get());
